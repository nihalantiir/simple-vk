...
- `SIMPLE_VK_IMGUI` CMake option (default `ON`). When `OFF`, Dear ImGui is
  never fetched (no network access needed at configure), `src/debug/` is
  not compiled, and `main.cpp` runs the frame loop without `DebugUi`.
  Documented in the README Build section and the wiki's Build/Libraries
  pages
